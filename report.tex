...
\begin{figure}
  \centering
  \includegraphics{fiber_bed_diag.pdf}
...
\begin{figure}
  \centering
  \includegraphics{consol_fig.pdf}
...
\begin{align}
  ...
  &= \spring {\frac{\disp{i+1}{j} - \disp{i}{j}}{\left(\dx + \disp{i+1}{j} - \disp{i}{j} + \cO{\dx^2}\right){\left(\frac{1}{\vzero} + \frac{\disp{i+1}{j} - \disp{i}{j}}{\vzero\dx} -\frac{1}{\vmax}  + \cO{\dx}\right)^4}}}%
    + \cO{\dx} % =  \nonumber \\
  % &= \spring \frac{\disp{i+1}{j} - \disp{i}{j}}{\left(\dx + \disp{i+1}{j} - \disp{i}{j}    + \cO{\dx^2}\right){\left(\frac{1}{\vzero} + \frac{\disp{i+1}{j} - \disp{i}{j}}{\vzero\dx} -\frac{1}{\vmax}  + \cO{\dx}\right)^4} }+ \cO{\dx}
     % =  \nonumber \\
...
% %
% \begin{enumerate}
% \item constant fiber volume fraction of $\vfrac = 0.6$, temperature $\temp=\QM$, and degree of cure $\cure$; this gives a linear model with appropriate physical units, \label{en:all-const}
% \item fiber volume fraction dependent on the deformation according to formula \eqref{eq:fiber-volume}, the remaining variables as in previous case,
