...
\begin{align}
  \efstress = \spring
  \frac{\frac{\vfrac}{\vzero} -1}{\left(\frac{1}{\vfrac} -\frac{1}{\vmax}\right)^4}, \label{eq:stress-nonlin}
\end{align}
%
...
%
\note{reproduce the derivation of this formula from  Hubert thesis}
%
\begin{align}
  \vfrac(\strain) = \frac{\vzero}{1+\strain(x)} \label{eq:fibre-volume}
\end{align}
%
where $\strain =  u_{,x}$.
...
%
\begin{align}
 \press_{,x} =\frac{\viscos(\temp,\cure)}{\permeab}    u_{,t}(x,t) ,  \label{eq:darcy-link}
\end{align}
%
...
%
\begin{align}
  \disp{i}{j+1}  =   \disp{i}{j} + \dt\frac{\permeab}{\viscos_i}\left(\frac{\disp{i+1}{j} - 2\disp{i}{j} + \disp{i-1}{j}}{\dx^2}   + f(x_i) \right). 
\end{align}
%
